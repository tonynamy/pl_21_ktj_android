...
public class FacListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnEtcTaskplan = findViewById(R.id.btnEtcTaskplan);
        ...
        //기타작업 버튼 클릭시
        btnEtcTaskplan.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                View view = LayoutInflater.from(v.getContext()).inflate(R.layout.dialog_etc_taskplan, null);
                builder.setView(view);

                EditText eTextEtcPlanName = view.findViewById(R.id.eTextEtcPlanName);
                Spinner spinEtcPlanTeam = view.findViewById(R.id.spinEtcPlanTeam);
                TextView textDialogCancel12 = view.findViewById(R.id.textDialogCancel12);
                TextView textDialogSubmit12 = view.findViewById(R.id.textDialogSubmit12);
                AlertDialog dialog = builder.create();
                ...
                API.APICallback apiCallback = new API.APICallback() {
                    ...
                    public void onSuccess(Object data) {
                        ...
                        HintSpinnerAdapter adapter = new HintSpinnerAdapter(v.getContext(), R.layout.spinner_item, new ArrayList<>());
                        adapter.setDropDownViewResource(R.layout.spinner_item_drop);
                        adapter.isBlack = true;
                        spinEtcPlanTeam.setAdapter(adapter);
                        ...
                        for(int i = 0; i < teams.size(); i++) {
                            ...
                            //자동선택되는 부분은 여기에 해야함 spinEtcPlanTeam.setSelection(i); 이런식으로
                        }
                ...
                //취소 선택시
                textDialogCancel12.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        dialog.dismiss();
                    }
                ...
                //확인 선택시
                textDialogSubmit12.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        ...
                        API.APICallback apiCallback = new API.APICallback() {
                            @Override
                            public void onSuccess(Object data) {
                                Toast.makeText(v.getContext(), "작업계획 등록에 성공했습니다", Toast.LENGTH_SHORT).show();
                        ...
                        API api = new API.Builder(apiCallback).build();
                        api.editTaskPlan(place_id, eTextEtcPlanName.getText().toString(), teams.get(spinEtcPlanTeam.getSelectedItemPosition()).id, 4);
